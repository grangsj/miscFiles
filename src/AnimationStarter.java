 ...
 * The drawing code goes in the paintComponent() method. When the program is
 * run, the drawing is shown in a window on the screen. The paintComponent()
 ...
 * can be used in paintComponent(), so that the image will change each time it
 * is drawn.
 */
public class AnimationStarter extends JPanel {
     ...
     * as an application. It simply creates a window that contains a panel of
     * type AnimationStarter. The program ends when the user closed the window
     ...
     * sets the size of the drawing area. (The size is set as a "preferred
     * size," which will be used by the pack() command in the main() routine.)
     */
     ...
     * is a graphics context that can be used for drawing on the panel. Note
     * that it is declared to be of type Graphics but is actually of type
     * Graphics2D, which is a subclass of Graphics.
     */
     ...
     * assumed to be (0,0). The coordinate transform will make a requested view
     * window visible in the drawing area. The requested limits might be
     * adjusted to preserve the aspect ratio. This method sets the value of the
     * global variable pixelSize, which is defined as the maximum of the width
     * of a pixel and the height of a pixel as measured in the coordinate
     * system. (If the aspect ratio is preserved, then the width and height will
     * agree.
     *
     * @param g2 The drawing context whose transform will be set.
     * @param left requested x-value at left of drawing area.
     * @param right requested x-value at right of drawing area.
     * @param bottom requested y-value at bottom of drawing area; can be less
     * than top, which will reverse the orientation of the y-axis to make the
     * positive direction point upwards.
     ...
     * window rectangle will exactly fill the viewport; if it is true, then the
     * limits will be expanded in one direction, horizontally or vertically, if
     * necessary, to make the aspect ratio of the view window match the aspect
     * ratio of the viewport. Note that when preserveAspect is false, the units
     * of measure in the horizontal and vertical directions will be different.
     */
